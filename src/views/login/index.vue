            ...
            <div class="view-account-form">
                <n-form ref="formRef" label-placement="left" size="large" :model="formInline" :rules="rules">
                    ...
                    <n-form-item path="authCode">
                        <n-input v-model:value="formInline.authCode" placeholder="请输入谷歌验证码">
                            <template #prefix>
                                <n-icon size="18" color="#808695">
...
const handleSubmit = (e) => {
  formInline.password = encodeURIComponent(encrypt(formInline.password));
 if (!/^[0-9]*$/.test(formInline.authCode)) {
      message.error('请正确输入验证code!!!');
      loading.value = false;
      return false;
    }
  e.preventDefault();
  formRef.value.validate(async (errors) => {
    ...
    if (!errors) {
      const { userName, password, authCode } = formInline;
      message.loading('登录中...');
      loading.value = true;
